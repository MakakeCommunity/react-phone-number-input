...
describe("input-control", () => {
  it("should get pre-selected country", () => {
    ...
    getPreSelectedCountry({}, null, ["US", "RU"], false, metadata).should.equal(
      "US"
    );

    // Can return "International".
    // Country can't be derived from the phone number.
    expect(getPreSelectedCountry({}, undefined, ["US", "RU"], true, metadata))
      .to.be.undefined;
  ...
  it("should generate country select options", () => {
    ...
    getCountrySelectOptions(
      ["US", "RU"],
      { ...defaultLabels, RU: "Russia", ZZ: "Intl" },
      true
    ).should.deep.equal([
      {
        label: "Intl"
  ...
  it("should migrate parsed input for new country", () => {
    ...
    migrateParsedInputForNewCountry("+99", "KG", "US", metadata).should.equal(
      "+1"
    );
  ...
  it("should format phone number in e164 or fallback to value", () => {
    // No number.
    e164().should.be.empty;

    // International number. Just a '+' sign.
    // expect(e164("+")).should.equal("+");

    // International number.
    e164("+7800").should.equal("+7800");

    // National number. Without country.
    e164("8800", null).should.equal("8800");

    // National number. With country. Just national prefix.
    // expect(e164("8", "RU", metadata)).should.equal("8");
  ...
  it("should get country for parsed input", () => {
    ...
    expect(
      getCountryForParsedInput("+", undefined, ["US", "RU"], true, metadata)
    ).to.be.undefined;
    ...
    expect(getCountryForParsedInput("+7", "FR", ["FR", "RU"], true, metadata))
      .to.be.undefined;
    expect(
      getCountryForParsedInput("+7800", "FR", ["FR", "RU"], true, metadata)
    ).to.be.undefined;
